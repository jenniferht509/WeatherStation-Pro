...
Ajouter un module Wi‑Fi (Pico W) pour envoyer les données météo vers une interface web.
Intégrer une carte SD pour sauvegarder l’historique des mesures.
Optimiser le code avec des interruptions matérielles pour les boutons (meilleure réactivité).
Ajouter une calibration automatique du capteur de gaz pour plus de précision.

Difficultés rencontrées
Lecture des deux DHT22 : nécessité d’ajouter un délai pour éviter les erreurs de mesure.
Synchronisation des alertes (LED, buzzer, servo) pour qu’elles soient cohérentes et visibles.
Ajustement des seuils du potentiomètre pour que la différence entre zones soit bien détectée.
Configuration initiale de l’OLED (adresse I2C et câblage SDA/SCL) dans Wokwi.
